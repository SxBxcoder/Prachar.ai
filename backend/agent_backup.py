...
def generate_copy(campaign_plan: Dict[str, str], user_id: str) -> List[str]:
    ...
    Cost-optimized with throttling protection and fallback.
...
Generate the captions now:"""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Handle ThrottlingException (429)
                if error_code == 'ThrottlingException':
                    if attempt < max_retries:
                        ...
                        print(f"❌ Throttled after {max_retries + 1} attempts. Using Demo Mode.")
        ...
        print("❌ All retries exhausted. Using Demo Mode.")
        return demo_captions
        ...
        print(f"\n{'='*60}")
        print("🔄 Returning Demo Mode fallback captions...")
        print(f"{'='*60}\n")
        
        # Return demo captions so frontend doesn't crash
        return [
            f"🔥 {campaign_plan.get('hook', 'Exciting news')}! {campaign_plan.get('offer', 'Special offer')} - {campaign_plan.get('cta', 'Check it out')}! 💯",
            f"✨ {campaign_plan.get('hook', 'Big announcement')}! {campaign_plan.get('offer', 'Limited time')} - {campaign_plan.get('cta', 'Do not miss')}! 🎉",
            f"💥 {campaign_plan.get('hook', 'Amazing update')}! {campaign_plan.get('offer', 'Exclusive deal')} - {campaign_plan.get('cta', 'Join now')}! 🚀"
...
@tool
def generate_image(caption: str, user_id: str, brand_colors: Optional[List[str]] = None) -> str:
    """
    Generate a campaign poster using Titan Image Generator v1.
    Incorporates brand colors from guidelines.
...
Style: Contemporary Indian social media poster"""
    ...
    # Step 3: Call Bedrock Titan Image Generator
    try:
        request_body = {
            "taskType": "TEXT_IMAGE",
            "textToImageParams": {
...
Execute the complete campaign now."""
        ...
        campaign_plan = extract_plan_from_response(agent_response)
        captions = extract_captions_from_response(agent_response)
        image_url = extract_image_url_from_response(agent_response)
        ...
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Campaign generation failed', 'details': str(e)})
        }


def extract_plan_from_response(response) -> Dict[str, str]:
    """Extract campaign plan from agent response."""
...
def extract_captions_from_response(response) -> List[str]:
    """Extract generated captions from agent response."""
    ...
    # Return extracted captions or defaults
    return captions[:3] if len(captions) >= 3 else default_captions


def extract_image_url_from_response(response) -> str:
    """Extract image URL from agent response."""
    response_text = str(response) if response else ""
    ...
    # Return first S3 URL found, or placeholder
    for url in urls:
        if 's3.amazonaws.com' in url or '.png' in url or '.jpg' in url:
            return url
    
    # Placeholder image URL
    return "https://via.placeholder.com/1024x1024/4F46E5/FFFFFF?text=Campaign+Poster"
